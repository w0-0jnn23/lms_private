    ...
    while start <= end:
        ...
        if find_name in output[0] and find_date in output[1]:
            ...
            while find_date == output[1]:
                ...
                if temp:
                    output = temp
                    print(output[0], find_name, output[0] == find_name)
...
def search(id):
    ...
    name_date = soup.find_all(attrs={'class':'blue01'})
    
    if not name_date:
        return None
    
    name = get_name(name_date)
    date = get_date(name_date)
...
find_name = input("찾을 게시글의 작성자를 입력하세요: ")
